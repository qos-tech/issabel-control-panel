...
function renderCard(item) {
    ...
    const otherParty = item.other_party || '';
    const calleridNum = item.callerid_num || '';
    const calleridName = item.callerid_name || '';
    const connectedLineNum = item.connected_line_num || '';
    const connectedLineName = item.connected_line_name || '';
    const currentExten = item.current_exten || '';
    const currentContext = item.current_context || '';
    const currentApplication = item.current_application || '';
    ...
    const titleParts = [
        ...
        `Status: ${label}`,
        directionLabel && cardClass === 'busy' ? `Direção: ${directionLabel}` : '',
        ...
        currentApplication ? `Aplicação: ${currentApplication}` : '',
